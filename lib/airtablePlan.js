  ...
  try {
    ...
    if (typeof raw === "string" && raw.trim()) {
      ...
      if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
        if (parsed.v === 2) {
          savingsPlacements =
            parsed.placements && typeof parsed.placements === "object"
              ? parsed.placements
              : {};
          savingsSkipped = Array.isArray(parsed.skipped) ? parsed.skipped.map(String) : [];
        } else {
  ...
  return {
    ...
    paychecks: dedupeById(
      paychecks.map((r) => ({
        ...
        extraIncome: num(r.fields?.["Extra Income"]),
      }))
    ),
...
export async function savePlanToAirtable(state) {
  const paychecks = Array.isArray(state?.paychecks) ? state.paychecks : [];
  const items = Array.isArray(state?.items) ? state.items : [];
  const payAmount = num(state?.payAmount);

  // Never wipe a populated base with an empty/broken client payload.
  if (payAmount <= 0 && paychecks.length === 0 && items.length === 0) {
    const err = new Error(
      "Refusing to save empty plan (would wipe Airtable). Reload or restore seed first."
    );
    err.statusCode = 400;
    throw err;
  }
  ...
  const placements =
    state.savingsPlacements && typeof state.savingsPlacements === "object"
      ...
      : {};
  const skipped = Array.isArray(state.savingsSkipped) ? state.savingsSkipped : [];
  await replaceBySourceId(TABLES.planSettings, [
    {
      sourceId: "default",
      payAmount,
      savingsPlacements: JSON.stringify({
        v: 2,
        placements,
        skipped,
      }),
  ...
  const paycheckMap = await replaceBySourceId(
    ...
    (state.paychecks || []).map((p, i) => ({
      sourceId: p.id,
      label: p.label,
      extraIncome: num(p.extraIncome),
      sortOrder: i,
    })),
    (row) => ({
      Label: row.label,
      "Extra Income": row.extraIncome || null,
      "Sort Order": row.sortOrder,
    })
  );
